...
class Plugin {
	...
	public static function checkDependencies( bool $allow_exit = false ): bool {
		if ( ! self::isPHPCompatible() ) {
			...
			$message = sprintf( esc_html__( '%1$s requires minimum PHP version %2$s', 'wll-loyalty-launcher' ), WLL_PLUGIN_NAME, WLL_MINIMUM_PHP_VERSION );
		...
		if ( ! self::isWordPressCompatible() ) {
			...
			$message = sprintf( esc_html__( '%1$s requires minimum WordPress version %2$s', 'wll-loyalty-launcher' ), WLL_PLUGIN_NAME, WLL_MINIMUM_WP_VERSION );
		...
		if ( ! self::isActive( 'woocommerce/woocommerce.php' ) ) {
			...
			$message = sprintf( esc_html__( '%1$s requires WooCommerce to be installed and activated in order to be used.', 'wll-loyalty-launcher' ), WLL_PLUGIN_NAME );
			$allow_exit ? exit( esc_html( $message ) ) : WC::adminNotice( esc_html( $message ), 'error' );
		...
		if ( ! self::isWooCompatible() ) {
			...
			$message = sprintf( esc_html__( '%1$s requires minimum Woocommerce version %2$s', 'wll-loyalty-launcher' ), WLL_PLUGIN_NAME, WLL_MINIMUM_WC_VERSION );
			$allow_exit ? exit( esc_html( $message ) ) : WC::adminNotice( esc_html( $message ), 'error' );
		...
		if ( ! self::isLoyaltyCompatible() ) {
			// translators: 1. %s will replace plugin name, 2. %s replace WooCommerce version
			$message = sprintf( esc_html__( '%1$s requires minimum loyalty version %2$s', 'wll-loyalty-launcher' ), WLL_PLUGIN_NAME, WLL_MINIMUM_WLR_VERSION );
			$allow_exit ? exit( esc_html( $message ) ) : WC::adminNotice( esc_html( $message ), 'error' );
	...
	protected static function isPHPCompatible(): bool {
		return (int) version_compare( PHP_VERSION, WLL_MINIMUM_PHP_VERSION, '>=' ) > 0;
	}
	...
	protected static function isWordPressCompatible(): bool {
		return (int) version_compare( get_bloginfo( 'version' ), WLL_MINIMUM_WP_VERSION, '>=' ) > 0;
	}
	...
	protected static function isWooCompatible(): bool {
		...
		return (int) version_compare( $woo_version, WLL_MINIMUM_WC_VERSION, '>=' ) > 0;
	}
	...
	protected static function getLoyaltyVersion(): string {
		if ( defined( 'WLL_PLUGIN_VERSION' ) ) {
			return WLL_PLUGIN_VERSION;
		}
